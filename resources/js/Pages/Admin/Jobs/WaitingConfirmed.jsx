...
export default function WaitingConfirmed({ auth }) {
    ...
    const edit = (finalized) => {
        ...
        editingDraft.breaks = JSON.parse(JSON.parse(finalized.breaks));
    ...
    function handleSubmit(e) {
        e.preventDefault()
        setLoading(true);
        axios.post('/jobs-editing', values).then((res) => {
            axios.get("/data-unconfirmed-jobs").then((res)=>{
                setData(res.data);
                setLoading(false);
                setShowEdit(false);
    ...
    function handleConfirm(e) {
        e.preventDefault()
        setLoading(true);
        axios.post('/jobs-confirmation', values).then((res) => {
            axios.get("/data-unconfirmed-jobs").then((res)=>{
                setData(res.data);
                setLoading(false);
                setShowEdit(false);
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Waiting Confirmed Jobs</h2>}
        >
            ...
            {showEdit && (
                <div>
                    ...
                    <div className='flex justify-evenly items-center my-10 pb-20'>
                        ...
                        <Button onClick={handleConfirm}   className=' bg-green-500 '>Onayla</Button>
                    </div>
