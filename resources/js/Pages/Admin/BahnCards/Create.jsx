...
const validationSchema = yup.object().shape({
    number: yup
        .string()
        .required("Bahn Karte Nummer erforderlich")
        .transform((value) => value.replace(/\s+/g, ""))
        .matches(
            /^\d{16}$/,
            "Bahn Karte Nummer muss genau 16 rakamdan oluşmalıdır"
        ),
    valid_from: yup.date().required("Gültig ab erforderlich"),
    valid_to: yup.date().required("Gültig bis erforderlich"),
    class: yup
        .string()
        .oneOf(["1", "2"], "Bahn Karte Klasse muss entweder 1 oder 2 sein")
        .required("Bahn Karte Klasse erforderlich"),
});
            ...
            <div className="container mx-auto mt-10">
                <Formik
                    ...
                    onSubmit={(values, { setSubmitting, resetForm }) => {
                        axios
                            .post(route("bahn-cards-store"), values)
                            .then((res) => {
                                if (res.data.success) {
                                    resetForm();
                                    Swal.fire({
                                    ...
                                    Swal.fire({
                                        ...
                                        text:
                                            data.data.message ||
                                            "Bahnkarte konnte nicht hinzugefügt werden!",
                                    });
                                }
                                setSubmitting(false);
                            })
